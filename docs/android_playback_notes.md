...
这份文档只针对 `packages/ktv_player` 当前这套播放器实现，目的是避免后续再次踩到“选文件闪退”“release 能装但不能播”“有声无画”这类回归。

## 1. 当前播放器结构

- 示例 UI 入口：`lib/main.dart`
- Android 播放控制器：`packages/ktv_player/lib/player/android_native_player_controller.dart`
- Dart 与原生通信：`packages/ktv_player/lib/player/platform_channel_player_controller.dart`
- Android 示例文件选择：`android/app/src/main/kotlin/com/ktv/player/ktv2_example/MainActivity.kt`
- Android 原生播放器宿主：`packages/ktv_player/android/src/main/kotlin/com/ktv/player/ktv2/NativeKtvPlayerHost.kt`
...
- `packages/ktv_player/android/src/main/c/native_vlc_bridge.c`
- `packages/ktv_player/android/src/main/c/CMakeLists.txt`
