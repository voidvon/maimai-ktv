...
class MainFlutterWindow: NSWindow {
  private var nativePlayerBridge: NativeKtvPlayerBridge?
  ...
  override func awakeFromNib() {
    ...
    let nativePlayerRegistrar = flutterViewController.registrar(forPlugin: "KtvNativePlayer")
    let nativePlayerBridge = NativeKtvPlayerBridge(
      messenger: flutterViewController.engine.binaryMessenger
    )
    nativePlayerRegistrar.register(
      NativeVideoViewFactory(bridge: nativePlayerBridge),
      withId: "ktv/native_video_view"
    )
    self.nativePlayerBridge = nativePlayerBridge
